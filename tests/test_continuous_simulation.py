...
class TestStatTestsSimulation(unittest.TestCase):
    ...
    def test_success(self):
        ...
        self.assertTrue(
            len(self.tests) == len(info.keys()), "Number of tests doesn't match with number of result dicts"
        )
        for test in self.tests:
            ...
            self.assertTrue(
                len(test_info["alpha_ci"]) == 2, f"Wrong confidence interval for alpha: {test_info['alpha_ci']}"
            )
            ...
            self.assertTrue(
                len(test_info["power_ci"]) == 2, f"Wrong confidence interval for power: {test_info['power_ci']}"
            )

            self.assertTrue("aa_pvalues" in test_info, f"AA p-values not in result dict")
            self.assertTrue(
                len(test_info["aa_pvalues"]) == experiments_num,
